    ...
    if(api_horario_atendimento()){
        ...
        if($api02){
           ...
           tracking($canal, $ddr, $ticket, $indice, 'CLIENTE EMPRESA - JA SOU CLIENTE', 'PERCURSO', 'FILA QUE A LIGACAO FOI DIRECIONADA: '.$fila);
            ...
            //17
            $indice++;
            tracking($canal, $ddr, $ticket, $indice, 'PESQUISA DE SATISFACAO - CLIENTE EMPRESA - JA SOU CLIENTE', 'PERCURSO', 'CLIENTE AINDA ESTA NA LINHA');
...
function M3_Usuario($uniqueid, $origem){
    ...
    if(api_horario_atendimento()){
        ...
        if($retorno->{'CLIENTE ATIVO'} == "S"){
            ...
            //33
            $indice++;
            tracking($canal, $ddr, $ticket, $indice, 'PESQUISA DE SATISFACAO - USUARIO', 'PERCURSO', 'CLIENTE AINDA ESTA NA LINHA');
